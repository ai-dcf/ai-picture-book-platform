...
import type { ModelDomain, ModelHealthState } from "@/modules/model-config/presentation/types";
...
export function useModelHealth() {
  ...
  function getHealthState(domain: ModelDomain, alias: string): ModelHealthState {
    const key = createKey(domain, alias);
    return healthStateMap[key] ?? { status: "untested" };
  }

  function isTesting(domain: ModelDomain, alias: string): boolean {
    const key = createKey(domain, alias);
    return testingKey === key;
  }
