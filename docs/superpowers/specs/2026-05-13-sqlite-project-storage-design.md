...
CREATE TABLE image_versions (
    ...
    image_url TEXT,
    description TEXT,                        -- 图片描述（自动生成+用户编辑）
    description_edited INTEGER NOT NULL DEFAULT 0, -- 描述是否由用户编辑过
    created_at INTEGER NOT NULL,
...
#### 上传/保存页面图片 (支持历史记录 + 自动描述)
```
POST /api/projects/:id/pages/:pageIndex/image
Content-Type: multipart/form-data
Request: { image: File, saveToHistory?: boolean = true, autoGenerateDescription?: boolean = true }
Response: { imageUrl: string, versionId: string, description: string }
```
...
Response: { versions: [{ id, versionNumber, createdAt, imageUrl, description }] }
```
...
Response: { success: true, imageUrl: string, description: string }
```
...
Content-Type: multipart/form-data
Request: { images: File[] }
Response: { groupId: string, candidates: [{ index, imageUrl, id, description }] }
```

#### 获取场景候选历史
```
GET /api/projects/:id/assets/scenes/:assetId/candidate-groups
Response: { groups: [{ id, createdAt, candidates: [{ id, index, imageUrl, description }] }] }
```
...
#### 自动生成图片描述
```
POST /api/images/:imageId/generate-description
Request: {}
...
#### 解析图片引用（文本转描述）
```
POST /api/text/parse-image-references
Request: { text: "大灰狼(图片1) 在森林里行走", projectId: string }
Response: { 
  parsedText: "一只灰色的狼在绿色的森林里行走",
  references: [
    { 
      match: "图片1", 
      imageId: "img_12345", 
      description: "一只灰色的狼，站立在草地上" 
    }
  ]
}
```
...
| `src/lib/db/repositories/image-repository.ts` | 图片版本管理（含描述） |
| `src/lib/db/repositories/asset-repository.ts` | 素材和候选图片管理（含描述） |
| `src/lib/image-description-service.ts` | 图片描述生成和解析服务 |
| `src/lib/image-reference-parser.ts` | 图片引用解析服务 |
...
| `src/app/api/images/[id]/description/route.ts` | 图片描述编辑/生成 |
| `src/app/api/text/parse-image-references/route.ts` | 图片引用解析 |
...
| `src/components/studio/ImageDescriptionEditor.tsx` | 图片描述编辑器组件（新增） |
| `src/components/studio/ImageReferenceToolbar.tsx` | 图片引用工具栏（新增） |
...
**设计目标**：
- 支持双层描述：素材级（已有）+ 图片级（新增）
- 支持自动生成描述 + 用户编辑
- 支持在文本中引用图片

**实现方案**：
1. **数据库层**：
   - `image_versions.description`：每张图片的具体描述
   - `image_versions.description_edited`：标记是否用户编辑过
   - `scene_candidates.description`：单个候选图片描述

2. **业务逻辑层**：
   - 图片上传时自动调用 AI 描述服务生成描述
   - 用户可在 UI 中编辑描述，编辑后覆盖自动生成的内容
   - 描述优先使用用户编辑版本，无则使用自动生成版本

3. **引用解析**：
   - 支持 `[图片1]`、`[img_12345]` 按 ID 引用
   - 支持 `[大灰狼]` 按素材名称引用
   - 解析器在发送 prompt 前自动将引用替换为实际描述
